      ...
      if (isColor(valNode)) {
        ...
        Object.keys(valNode.vals).forEach((subValNode) => {
          result = result.concat(
            extractColorsFromExpression(valNode.vals[subValNode])
          );
        });
      }
    });
...
export function normalizeColors(colors: StylusNode[], text: string[]) {
  return colors.map((color) => {
    ...
    if (color.nodeName === "rgba") {
      normalized.column = getRealColumn(
        (color as any).raw,
        text,
        normalized.lineno
      );
    ...
    if (color.nodeName === "call") {
      normalized.column = getRealColumn(color.name, text, normalized.lineno);
      normalized.background = `${color.name}(${buildCallValueFromArgs(
        (color as any).args
      )})`;
    }
...
export const updateDecoratorsWrapperDebounced = debounce(
  updateDecoratorsWrapper,
  DEBOUNCE_TIME
);

export function activateColorDecorations(): Disposable {
  const disposables: Disposable[] = [];
  const colorsDecorationType = window.createTextEditorDecorationType(
    decorationType
  );
